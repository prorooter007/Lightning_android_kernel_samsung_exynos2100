...
anykernel3(){
  ...
  if [ -e $DIR/arch/arm64/boot/Image ]; then
    ...
    zip -r9 $PARENT_DIR/exynos2100-r9sxxx_kernel_`cat $DIR/include/config/kernel.release`_`date '+%Y_%m_%d'`.zip * -x .git README.md *placeholder
    cd $DIR
    curl https://bashupload.com/exynos2100-r9sxxx_kernel_`cat $DIR/include/config/kernel.release`_`date '+%Y_%m_%d'`.zip --data-binary @exynos2100-r9sxxx_kernel_`cat $DIR/include/config/kernel.release`_`date '+%Y_%m_%d'`.zip
  fi
}
